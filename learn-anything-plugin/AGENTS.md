...
- `commands/` — User-facing command workflows for the plugin.
- `agents/` — Supporting agent prompt/contracts used by component skills.

NotebookLM contract update: the default remains link-only/no file pipeline. The only approved download/upload exception is YouTube subtitle/chapter sidecars produced by root `download_yt_subs.sh` and uploaded through `scripts/notebooklm/add-youtube-with-subs.mjs`; raw subtitle/chapter text must not be stored in durable state.
