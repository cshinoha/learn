...
For source-grounded lessons, factual explanations, document/book/video references, citation/timestamp requests, or disputed claims, use NotebookLM before writing substantive content when phase-scoped MCP access is available.

1. Read `notebooklm-manifest.json` and `citations.jsonl` when present. Use the manifest's notebook array for `cross_notebook_query`.
2. Ask NotebookLM for a compact answer with selected citations; do not pass a long raw NotebookLM answer into the lesson context.
3. Build or consume a compact retrieval pack containing only the synthesis, selected citations, source IDs, notebook IDs, timestamps/deep links, confidence, and limitations.
4. Insert citations, timestamps, section/page references, and deep links directly into the HTML lesson where they support substantive claims.
5. Append only citations actually used in the lesson to `citations.jsonl`; keep them normalized and short.
6. Update `teach/RESOURCES.md` with short annotations explaining why each resource is useful, its level, and its learning role.
7. For large lessons, automatically create 1–2 NotebookLM Studio artifacts by learning role when useful (for example quiz, study guide, flashcards, mind map, infographic, slide deck). Store only artifact metadata/links in the manifest; do not download artifacts by default.
8. If NotebookLM fails, attempt refresh/reconnect once. If it still fails, ask the user how to proceed. Do not silently fall back to uncited parametric explanation for source-grounded work.
